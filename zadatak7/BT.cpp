...
// Konstruktor.
Grid::Segment::Segment(double left, double right, Segment * parent) : mpt_l(left), mpt_r(right), mparent(parent){
    //std::cout << "Segment konstruktor\n" << std::endl;
    mseg_l.release();
    mseg_r.release();
}
// Da li je segment list (nema djece) ?
bool Grid::Segment::isLeaf() const{
    if( mseg_l.get() == nullptr && mseg_r.get() == nullptr )
        return true;
    return false;
}
// Profini segment. Ako već ima djecu ne radi ništa.
void Grid::Segment::refine(){
    if( isLeaf() ){
            //std::cout << "leaf...";
            //print();
...
bool Grid::Segment::isIn(double x) const{
...
Grid::Segment::GridLeafIterator::GridLeafIterator(Segment* root) : mRoot(root){
    mStack.reset(new std::stack<Segment*>);
    //stavi root na stog
    mStack.get()->push(mRoot);
    }
// operator inkrementiranja (dovoljna je prefiks verzija)
Grid::Segment::GridLeafIterator& Grid::Segment::GridLeafIterator::operator++(){    //dodano
...
bool operator==(Grid::iterator const & lhs, Grid::iterator const & rhs){
